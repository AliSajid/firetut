...
<AuthCheck>
    ...
    <form class="w-full max-w-screen-md">
        <div class="form-control w-full mx-auto text-center max-w-xs">
            <img src={previewURL ?? $userData?.photoURL ?? "https://placehold.co/256x256/png"} alt="Profile Photo" class="mx-auto"
            width="256" height="256" aria-hidden="true" />
        </div>
        <label for="profilePhoto" class="label">
            <span class="label-text">Pick a file</span>
        </label>
        <input type="file" class="file-input file-input-bordered w-full max-w-xs" accept="image/png, image/jpeg, image/gif, image/webp" name="profilePhoto" on:input={upload}/>
    </form>
